...
func main() {
	...
	descheduler := nova.NewDescheduler(config.DeschedulerConfig, monitor, keystoneAPI)
	descheduler.Init(nova.SupportedSteps, ctx, database, config.DeschedulerConfig)
	go descheduler.DeschedulePeriodically(ctx)
