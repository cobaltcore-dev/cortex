...
func (r *PrometheusDatasourceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, client.ObjectKey{
		Namespace: datasource.Spec.DatabaseSecretRef.Namespace,
		Name:      datasource.Spec.DatabaseSecretRef.Name,
	}, dbSecret); err != nil {
		log.Error(err, "failed to get database secret", "secretRef", datasource.Spec.DatabaseSecretRef)
		datasource.Status.Error = "failed to get database secret: " + err.Error()
		if err := r.Status().Update(ctx, datasource); err != nil {
			log.Error(err, "failed to update datasource status", "name", datasource.Name)
			return ctrl.Result{}, err
		}
	...
	authenticatedDB, err := db.AuthenticatedFromSecret(ctx, dbSecret.Data)
	if err != nil {
	...
	if datasource.Spec.SSOSecretRef != nil {
		...
		if err := r.Get(ctx, client.ObjectKey{
			Namespace: datasource.Spec.SSOSecretRef.Namespace,
			Name:      datasource.Spec.SSOSecretRef.Name,
		}, authSecret); err != nil {
			log.Error(err, "failed to get sso secret", "secretRef", datasource.Spec.SSOSecretRef)
			datasource.Status.Error = "failed to get sso secret: " + err.Error()
			if err := r.Status().Update(ctx, datasource); err != nil {
				log.Error(err, "failed to update datasource status", "name", datasource.Name)
				return ctrl.Result{}, err
			}
		...
		authenticatedHTTP, err = sso.AuthenticatedFromSecret(authSecret.Data)
		if err != nil {
			log.Error(err, "failed to create authenticated HTTP client", "secretRef", datasource.Spec.SSOSecretRef)
			datasource.Status.Error = "failed to create authenticated HTTP client: " + err.Error()
			if err := r.Status().Update(ctx, datasource); err != nil {
				log.Error(err, "failed to update datasource status", "name", datasource.Name)
				return ctrl.Result{}, err
			}
		...
		authenticatedHTTP = http.DefaultClient
	}
	...
	if err := r.Status().Update(ctx, datasource); err != nil {
		log.Error(err, "failed to update datasource status", "name", datasource.Name)
		datasource.Status.Error = "failed to update datasource status: " + err.Error()
		return ctrl.Result{}, err
	}
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(
			...
			builder.WithPredicates(predicate.NewPredicateFuncs(func(obj client.Object) bool {
				...
				return obj.(*v1alpha1.Datasource).Spec.Prometheus != nil
			})),
		).
		Complete(r)
}
