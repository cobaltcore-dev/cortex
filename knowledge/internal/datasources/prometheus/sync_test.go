...
func TestNewCombinedSyncer(t *testing.T) {
	...
	mockMonitor := datasources.Monitor{}
	mockMQTTClient := &mqtt.MockClient{}

	config := conf.DatasourcePrometheusConfig{
		Metrics: []conf.DatasourcePrometheusMetricConfig{
			{Type: "vrops_host_metric", Alias: "test_metric"},
		},
		Hosts: []conf.DatasourcePrometheusHostConfig{
...
func TestNewSyncerOfType(t *testing.T) {
	...
	mockMonitor := datasources.Monitor{}
	hostConf := conf.DatasourcePrometheusHostConfig{Name: "test_host"}
	metricConf := conf.DatasourcePrometheusMetricConfig{Alias: "test_metric"}
