...
- Reject `resize` requests from nova
- Added feature to allow multiple pipelines per scheduler
- Created `cortex-core` alerts that are being shared across all the bundles
- Created bundle specific alerts
- Host Utilization KPI was refactored to Host Capacity KPI
- Implemented improvements for Cortex Infrastructure Dashboard
  - All hosts without a `SAPPHIRE_RAPIDS` trait are now being considered as `cascade-lakes` in the infrastructure dashboard
...
- Added pipeline premodifier to test the implemented nova filters
...
- Clean up tests by replacing plain table names in sql queries with `Insert()` or `TableName()` method calls
- Added hypervisor status disabled as filter for the `feature_sap_host_details`
