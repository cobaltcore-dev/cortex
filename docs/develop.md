...
│   └── external/      # External API definitions (e.g. OpenStack services)
...
│   └── shim/          # Placement API shim (REST endpoints)
├── pkg/               # Reusable/public library code
├── helm/
│   ├── library/       # Generic cortex Helm chart (manager and dependencies)
│   ├── dev/           # Charts for local dev dependencies (e.g. kube-prometheus-stack)
│   └── bundles/       # Umbrella Helm charts for deploying Cortex in specific domains
