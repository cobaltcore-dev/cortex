...
| **[Delegation model](concepts/delegation-model.md)** | The pattern by which the platform calls Cortex for a placement recommendation and remains the authority that executes it. Advisory (reorder the platform's candidates) today; fuller delegation planned. |
...
| **[Remote cluster](concepts/multicluster.md)** | A cluster that stores selected CRDs/resources for Cortex, addressed per-GVK and routed by matching labels (commonly the availability zone). |
...
| **In-flight reservation** | Capacity blocked pessimistically for a VM currently being scheduled, so concurrent requests are not offered the same slot (double-booking). |
