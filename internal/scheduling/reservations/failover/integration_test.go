	...
	for _, res := range reservationsList {
		...
		if res.Status.FailoverReservation != nil {
			if _, exists := res.Status.FailoverReservation.Allocations[vmUUID]; exists {
				...
				env.T.Logf("VM %s has failover reservation %s on hypervisor %s", vmUUID, res.Name, res.Status.Host)
				return
			}
		}
	}
...
func newVM(uuid, flavorName, projectID, host string, memoryMB, vcpus uint64) VM {
	return VM{
		UUID:              uuid,
