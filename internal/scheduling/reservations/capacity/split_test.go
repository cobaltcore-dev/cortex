...
func TestSplitCapacity(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			free, assigned, unassigned, _, _ := SplitCapacity(tc.groups, tc.hosts)
...
func TestSplitCapacity_SumNeverExceedsTotal(t *testing.T) {
	...
	_, assigned, _, _, _ := SplitCapacity(groups, hosts)
...
func TestSplitCapacity_Deterministic(t *testing.T) {
	...
	_, first, firstUnassigned, _, _ := SplitCapacity(groups, hosts)
	for i := range 10 {
		_, got, gotUnassigned, _, _ := SplitCapacity(groups, hosts)
