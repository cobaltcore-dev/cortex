...
func (api *HTTPAPI) HandleReportUsage(w http.ResponseWriter, r *http.Request) {
	...
	// Build usage report response
	report, err := api.buildUsageReport(r.Context(), log, projectID, req.AllAZs)
	if err != nil {
		log.Error(err, "failed to build usage report", "projectID", projectID)
...
// buildUsageReport constructs the full usage report for a project.
func (api *HTTPAPI) buildUsageReport(
	ctx context.Context,
	log logr.Logger,
	projectID string,
	...
	commitmentsByAZFlavorGroup, err := api.buildCommitmentCapacityMap(ctx, log, projectID, flavorGroups)
	if err != nil {
	...
	vms, err := api.getProjectVMs(ctx, log, projectID, flavorGroups, allAZs)
	if err != nil {
	...
	vmAssignments, assignedToCommitments := api.assignVMsToCommitments(vms, commitmentsByAZFlavorGroup)

	// Step 5: Build the response
	report := api.buildUsageResponse(vms, vmAssignments, flavorGroups, allAZs)
...
func (api *HTTPAPI) buildCommitmentCapacityMap(
	ctx context.Context,
	log logr.Logger,
	projectID string,
	_ map[string]compute.FlavorGroupFeature, // flavorGroups not currently used but kept for future enrichment
	...
	if err := api.client.List(ctx, &allReservations, client.MatchingLabels{
...
func (api *HTTPAPI) getProjectVMs(
	ctx context.Context,
	log logr.Logger,
	projectID string,
	...
	// Query VMs from Nova
	servers, err := api.novaClient.ListProjectServers(ctx, projectID)
	if err != nil {
...
func (api *HTTPAPI) assignVMsToCommitments(
	vms []VMUsageInfo,
...
func (api *HTTPAPI) buildUsageResponse(
	vms []VMUsageInfo,
