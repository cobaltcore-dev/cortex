...
//
//  1. CR headroom (RAM only): checks if any CommittedResource CRD for the project has
//     unused capacity that can accommodate the request
//     (AcceptedSpec.Amount - Status.UsedResources >= requestRAM).
//     If a matching CR has headroom, the request is accepted immediately — PAYG checks
//     are skipped entirely.
//
//  2. PAYG headroom (ram, cores, instances): for each resource that has quota set in
//     the ProjectQuota CRD, checks headroom. Resources without quota are not enforced.
//     For RAM: headroom = Quota - sum(CR amounts) - PaygUsage (CRs reserve RAM capacity).
//     For cores/instances: headroom = Quota - PaygUsage (no CR deduction).
//
...
func (s *FilterQuotaEnforcement) Run(traceLog *slog.Logger, request api.ExternalSchedulerRequest) (*lib.FilterWeigherPipelineStepResult, error) {
	...
	// Step 5: List CommittedResource CRDs and check CR headroom (RAM only).
	// CRs only cover RAM, so a matching CR with free capacity accepts the entire request
	// (bypassing PAYG checks for cores/instances).
	...
	for i := range crList.Items {
		...
		now := time.Now()
		if spec.StartTime != nil && spec.StartTime.After(now) {
			continue
		}
		if spec.EndTime != nil && spec.EndTime.Time.Before(now) {
			continue
		}
	...
	// If not found, no quota is configured — skip enforcement (accept).
	// On transient errors, return error to fail-open (pipeline skips this filter).
	...
	// Only resources with quota explicitly set in the CRD are enforced — unset means unlimited.
