...
func TestResourceBalancingStep_Run(t *testing.T) {
	...
	_, err = testDB.Exec(`
        INSERT INTO feature_host_utilization (compute_host, availability_zone, ram_utilized_pct, vcpus_utilized_pct, disk_utilized_pct, total_memory_allocatable_mb, total_vcpus_allocatable, total_disk_allocatable_gb)
        VALUES
            ('host1','zone1', 0, 0, 0, 1000, 100, 100),
            ('host2','zone2',100, 100, 100, 1000, 100, 100)
    `)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
