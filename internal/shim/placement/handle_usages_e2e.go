...
// e2eTestUsages exercises GET /usages as a passthrough to upstream placement,
// confirming the shim forwards the request and returns a 2xx status.
func e2eTestUsages(ctx context.Context, _ client.Client) error {
	...
	req, err := http.NewRequestWithContext(ctx,
		http.MethodGet, sc.Endpoint+"/usages?project_id=e2e", http.NoBody)
	if err != nil {
		return err
	}
	req.Header.Set("X-Auth-Token", sc.TokenID)
	req.Header.Set("OpenStack-API-Version", "placement 1.9")
