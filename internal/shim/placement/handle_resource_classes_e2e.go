...
// e2eTestResourceClasses exercises GET /resource_classes as a passthrough to
// upstream placement, confirming the shim forwards the request and returns a
// 2xx status.
func e2eTestResourceClasses(ctx context.Context, _ client.Client) error {
	log := logf.FromContext(ctx)
	log.Info("Running resource_classes endpoint e2e test")
