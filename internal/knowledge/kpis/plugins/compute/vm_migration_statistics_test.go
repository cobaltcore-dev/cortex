...
func TestVMMigrationStatisticsKPI_Collect(t *testing.T) {
	...
	vmHostResidency, err := v1alpha1.BoxFeatureList([]any{
		&compute.VMHostResidencyHistogramBucket{FlavorName: "small", Bucket: 60, Value: 100, Count: 10, Sum: 600},
		&compute.VMHostResidencyHistogramBucket{FlavorName: "medium", Bucket: 120, Value: 200, Count: 20, Sum: 2400},
		&compute.VMHostResidencyHistogramBucket{FlavorName: "large", Bucket: 180, Value: 300, Count: 30, Sum: 5400},
	})
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
