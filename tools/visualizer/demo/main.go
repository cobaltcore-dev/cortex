...
func main() {
	...
	pipeline := flag.String("pipeline", "nova-external-scheduler", "The pipeline to use (default: 'nova-external-scheduler')")
	...
	client.Subscribe(map[string]string{
		"nova": "cortex/scheduler/nova/pipeline/finished",
	}[*schedulerType], 2, func(client mqtt.Client, msg mqtt.Message) {
		var req PipelineRequest
		switch *schedulerType {
		case "nova":
			var data struct {
				Request ExternalSchedulerRequest `json:"request"`
			}
			must.Succeed(json.Unmarshal(msg.Payload(), &data))
			data.Request.Pipeline = *pipeline
			req = data.Request
		}
		for {
