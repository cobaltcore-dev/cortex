...
func TestClient_ServedAZs(t *testing.T) {
	scheme := newTestScheme(t)

	t.Run("returns nil for no remote clusters", func(t *testing.T) {
		c := &Client{
			HomeCluster: newFakeCluster(scheme),
			HomeScheme:  scheme,
			homeGVKs:    map[schema.GroupVersionKind]bool{configMapGVK: true},
		}
		azs := c.ServedAZs(configMapGVK)
		if azs != nil {
			t.Errorf("expected nil, got %v", azs)
		}
	})

	t.Run("returns nil when labels have no availabilityZone key", func(t *testing.T) {
		c := &Client{
			HomeCluster: newFakeCluster(scheme),
			HomeScheme:  scheme,
			remoteClusters: map[schema.GroupVersionKind][]remoteCluster{
				configMapGVK: {
					{cluster: newFakeCluster(scheme), labels: map[string]string{"region": "eu"}},
				},
			},
		}
		azs := c.ServedAZs(configMapGVK)
	...
	t.Run("returns AZ set from remote cluster labels", func(t *testing.T) {
		c := &Client{
			HomeCluster: newFakeCluster(scheme),
			HomeScheme:  scheme,
			remoteClusters: map[schema.GroupVersionKind][]remoteCluster{
				configMapGVK: {
					{cluster: newFakeCluster(scheme), labels: map[string]string{"availabilityZone": "az-1"}},
					{cluster: newFakeCluster(scheme), labels: map[string]string{"availabilityZone": "az-2"}},
					{cluster: newFakeCluster(scheme), labels: map[string]string{"availabilityZone": "az-3"}},
				},
			},
		}
		azs := c.ServedAZs(configMapGVK)
		...
		for _, az := range []string{"az-1", "az-2", "az-3"} {
			if !azs[az] {
				t.Errorf("expected %s to be served", az)
			}
		}
		if azs["az-4"] {
			t.Error("az-4 should not be served")
		}
	})
}
