...
import (
	...
	schedulerNova "github.com/cobaltcore-dev/cortex/scheduling/internal/nova"
...
func NewAPI(config conf.SchedulerConfig, registry *monitoring.Registry, db db.DB) HTTPAPI {
	...
	for _, pipelineConf := range config.Nova.Pipelines {
		...
		pipelines[pipelineConf.Name] = schedulerNova.NewPipeline(
			pipelineConf, db, monitor.SubPipeline("nova-"+pipelineConf.Name),
		)
	}
...
func (httpAPI *httpAPI) HandleCommitmentChangeRequest(w http.ResponseWriter, r *http.Request) {
	...
	for _, commitmentChangeset := range requestData.ByProject {
		...
		for resourceName, resourceCommitmentChangeset := range commitmentChangeset.ByResource {
			...
			novaFlavorSpec := delegationAPI.NovaFlavor{
			...
			novaSpec := delegationAPI.NovaSpec{
				Flavor:           delegationAPI.NovaObject[delegationAPI.NovaFlavor]{Data: novaFlavorSpec},
				ProjectID:        projectMeta.UUID,
				AvailabilityZone: string(requestData.AZ),
				NumInstances:     uint64(nNewVMs), // Guaranteed non-negative.
			}
			novaRequestContext := delegationAPI.NovaRequestContext{
				ProjectID:       projectMeta.UUID,
				ProjectDomainID: projectMeta.Domain.UUID,
			}
			hosts, err := pipeline.Run(api.PipelineRequest{
				Spec:    delegationAPI.NovaObject[delegationAPI.NovaSpec]{Data: novaSpec},
