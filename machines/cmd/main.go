...
func main() {
	...
	wrappers := []scheduling.StepWrapper[controller.MachinePipelineRequest, struct{}]{
	...
	pipelines := make(
		map[string]scheduling.Pipeline[controller.MachinePipelineRequest],
		len(config.Machines.Pipelines),
	)
	for _, pipelineConf := range config.Machines.Pipelines {
		plugins := pipelineConf.Plugins
		pipelines[pipelineConf.Name] = scheduling.NewPipeline(
			controller.SupportedSteps, plugins, wrappers,
